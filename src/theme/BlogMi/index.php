...
<head>
  ...
  <meta name="author" content="BlogMi" />
  <link rel="icon" href="<?php app_site_link(); ?>/favicon.ico" />
  <title><?php if (app_is_post() || app_is_page()) {
    app_post_title(); ?> | <?php app_site_name();
  } else {
    app_site_name(); ?> | <?php app_site_desc();
  } ?></title>
...
<body>
  <div id="main">
    <div id="header">
      <div id="sitename"><a href="<?php app_site_link(); ?>" title="<?php app_site_desc(); ?>"><?php app_site_name(); ?></a></div>
    </div>
    <div class="clear"></div>
    <div id="sidebar">
      <div class="photo"><img src="<?php app_theme_url('photo.jpg'); ?>"></div>
      <div class="about"><?php app_site_desc(); ?></div>
      <div id="navbar">
        <a href="<?php app_site_link(); ?>" class="home" title="首页">首页</a>
        <a href="<?php app_get_url('archive'); ?>" class="archive" title="文章存档">文章存档</a>
        <a href="<?php app_get_url('about'); ?>" class="contact" title="联系方式">联系方式</a>
    ...
    <div id="content">
      <div id="content_box">
        ...
        <?php if (app_can_comment()) { ?>
          <div id="comm"><?php app_comment_code(); ?></div>
        <?php } ?>
                ...
                <tbody>
                  <tr>
                    <td width="140" style="vertical-align:top;">
                      ...
                      <ul id="list"><?php app_date_list(); ?></ul>
                    </td>
                    <td width="140" style="vertical-align:top;">
                      <h1 class="tag_list">标签</h1>
                      <ul id="list"><?php app_tag_list(); ?></ul>
                    </td>
                  </tr>
                </tbody>
              </table>
        ...
        <?php } else { ?>
          <?php if (app_is_tag()) { ?>
            <div id="page_info"><span><?php app_tag_name(); ?></span></div>
          <?php } else if (app_is_date()) { ?>
            <div id="page_info"><span><?php app_date_name(); ?></span></div>
          <?php } ?>
      ...
      <div class="clear"></div>
      <div id="page_bar">
        <?php if (app_has_new()) { ?><?php app_goto_new('«'); ?><?php } ?>
        <?php if (app_has_old()) { ?><?php app_goto_old('»'); ?><?php } ?>
      </div>
    <?php } ?>
    </div>
  </div>
